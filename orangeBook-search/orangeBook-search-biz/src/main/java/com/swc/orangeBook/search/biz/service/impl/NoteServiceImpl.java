package com.swc.orangeBook.search.biz.service.impl;
...
import com.swc.framework.common.util.DateUtil;
import com.swc.framework.common.util.NumberUtils;
import com.swc.orangeBook.search.biz.domain.mapper.SelectMapper;
import com.swc.orangeBook.search.biz.enums.NotePublishTimeRangeEnum;
import com.swc.orangeBook.search.biz.enums.NoteSortTypeEnum;
import com.swc.orangeBook.search.biz.index.NoteIndex;
    ...
    public PageResponse<SearchNoteRspVO> searchNote(SearchNoteReqVO searchNoteReqVO) {
        ...
        if (Objects.nonNull(notePublishTimeRangeEnum)) {
            ...
            switch (notePublishTimeRangeEnum) {
                case DAY ->
                        startTime = DateUtil.localDateTime2String(LocalDateTime.now().minusDays(1)); // 一天之前的时间
                case WEEK ->
                        startTime = DateUtil.localDateTime2String(LocalDateTime.now().minusWeeks(1)); // 一周之前的时间
                case HALF_YEAR ->
                        startTime = DateUtil.localDateTime2String(LocalDateTime.now().minusMonths(6)); // 半年之前的时间
            }
            // 设置时间范围
