    ...
    public void fansCountShardingJobHandler() throws Exception {
        ...
        for (; ; ) {
            ...
            userIds.forEach(userId -> {
                // 2: 对 t_following 关注表执行 count(*) 操作，获取粉丝总数
                int followingTotal = selectMapper.selectCountFromFansTableByUserId(userId);

                // 3: 更新 t_user_count 表, 更新对应 Redis 缓存
                int count = updateMapper.updateUserFansTotalByUserId(userId, followingTotal);
                // 更新对应 Redis 缓存
                if (count > 0) {
